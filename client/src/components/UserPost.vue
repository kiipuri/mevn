<template>
  <v-card v-if="isDataFetched" class="my-3">
    <router-link
      :to="'/post/' + post._id"
      style="width: 100%; height: 100%; position: absolute"
    />
    <div class="px-4">
      <v-card-title class="text-subtitle-1 mt-1 px-0">
        <v-avatar class="mr-2">
          <v-img
        ...
        <router-link :to="'/users/' + post.userID" class="user-link prel">
          {{ user.username }}
        </router-link>
        <span class="float-right font-weight-regular">
          {{ formatDate(post.createdAt) }}
        </span>
      </v-card-title>
      <v-card-text class="text-subtitle-1 post-text mb-2 pa-0 prel dinl">
        {{ post.post }}
      </v-card-text>
      <v-card-subtitle
        class="text-subtitle-2 font-weight-bold actions pa-0 pointer-none"
      >
        <v-btn
...
export default {
  ...
  methods: {
    ...
    getLikes() {
      axios
        .get(`http://localhost:9000/api/get-post-likes/${this.post._id}`)
        .then((res) => {
